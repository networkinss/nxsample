...
Add a Maven library:  
```nx generate @jnxplus/nx-maven:library samplelib --projects nxsample --directory libs```


## Start the application

Run `npx nx serve nxsample` to start the development server. Happy coding!

## Build for production

Run `npx nx build nxsample` to build the application. The build artifacts are stored in the output directory (e.g. `dist/` or `build/`), ready to be deployed.
